...
export const DEMO_STATS = {
  ...
  auto_locked_value_crore: 878.5,
  _auto_locked_source: "DEMO_CONSTANT — sum of frozen tender estimated_value_crore in production",
  tenders_frozen: 3,
  ...
  try {
    ...
    if (!tendersResult.error && tendersResult.data && tendersResult.data.length > 0) {
      ...
      const stats = {
        ...
        ai_alerts_active: alerts.length > 0 ? alerts.length : frozen.length + tenders.filter(t => (t.risk_score || 0) >= 80).length,
        critical_alerts: alerts.length > 0 ? alerts.filter(a => a.risk_level === 'CRITICAL').length : frozen.length,
        blockchain_tx_total: (auditResult.count || 0) + 1, blockchain_tx_today: auditResult.count || 0,
        auto_locked_value_crore: frozen.reduce((sum, t) => sum + (t.estimated_value_crore || 0), 0),
        _auto_locked_source: "COMPUTED — sum of frozen tender estimated_value_crore",
